 ...
 */
function PackageNodeModule({dev, dist, npmFolder, modules, isLiveUpdate}){
	...
	return through2.obj(function(file, enc, cb){
		...
		// 去除开发目录的绝对路径
		const fileDeep     = fileFullPath.replace(fileBasePath, '').replace(/\\/g, '/')
		// 计算出当前文件相对开发目录的层级数
		...
		const results = replaceStr.replace(replaceReg, (n) => {
			...
			// 只有传递了此变量才进行处理 
				...
				let hasModule = false;
				modules.forEach((n, i) => {
					const mPath = path.join(deepStr, n, moduleName)
					const bPath = path.join(dev, n, moduleName)
					const fname = moduleName.split('/').slice(-1)[0]
					const gPath = `${bPath}?(?(**{index,${fname}}).{js,json})`
					const files = glob.sync(gPath)
					// 判断当前文件是否存在
					if (files[0]){
						hasModule   = true
						npmDirctory = mPath;
